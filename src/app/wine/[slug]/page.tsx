...
export function generateStaticParams() {
  return getSlugs("wine").map((slug) => ({ slug }));
}

export default function WinePostPage({
  params,
}: {
  params: { slug: string };
}) {
  const { slug } = params;
  const post = getPost("wine", slug);
  if (!post) notFound();
      ...
      {post.body ? (
        ...
        <div className="prose prose-ink mt-6 max-w-none">
          ...
          <p className="mt-4 text-mute">
            Full article content can go here. Add a .md file in content/wine/ to
            render body content.
          </p>
        </div>
      )}
